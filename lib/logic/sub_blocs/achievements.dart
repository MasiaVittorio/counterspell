...
class CSAchievements extends BlocBase {
  ...
  CSAchievements(this.parent) {
    ...
    map = BlocMap<String?, Achievement>(Achievements.map,
  ...
  void checkNewAchievements([bool forceResetDev = false]) {
    ...
    for (final entry in Achievements.map.entries) {
      map.value[entry.key] = (forceResetDev)
          ? entry.value
          : map.value[entry.key]?.updateStats(entry.value) ?? entry.value;
    }
    map.refresh();
  ...
  bool achieve(String shortTitle, String? key) {
    ...
    final Achievement? oldAchievement =
        map.value[shortTitle] ?? Achievements.mapQuality[shortTitle];
  ...
  bool incrementBy(String shortTitle, int by) {
    ...
    final Achievement? oldAchievement =
        map.value[shortTitle] ?? Achievements.mapQuantity[shortTitle];
  ...
  bool reset(String shortTitle, {bool force = false}) {
    final Achievement? achievement =
        map.value[shortTitle] ?? Achievements.map[shortTitle];
  ...
  void playGroupEdited(bool fromClosedPanel) {
    ...
    achieve(Achievements.uiExpertShortTitle,
        fromClosedPanel ? "Playgroup panel" : "Playgroup menu");
  }

  void counterChosen(String? newCounterLongName) =>
      achieve(Achievements.countersShortTitle, newCounterLongName);

  void flippedOrRolled() => increment(Achievements.rollerShortTitle);
}
