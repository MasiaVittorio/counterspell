...
import 'package:counter_spell/data/tutorial/gestures.dart';
...
class CSTutorial {
  ...
  void showTutorial(int? index) async {
    ...
    hints = Tutorials.base.checkIndex(index)
        ...
        : [
            TutorialCards.first,
            for (final tutorial in Tutorials.base) ...tutorial.hints,
            TutorialCards.last,
          ];
  ...
  void handle(Hint hint, HighlightController? selfHighlight, int index) async {
    ...
    if (hint.autoHighlight != null) {
      ...
      await Future.delayed(const Duration(milliseconds: 1000));
      for (final a in [
        for (int i = 0; i < hint.repeatAuto; i++) h,
      ].separateWith(i)) {
        await a();
      }
    }
