...
class Player {
  ...
  factory Player.fromJson(Map<String, dynamic> json) => Player(
    ...
    states: [
      for (final stateJson in json["states"]) PlayerState.fromJson(stateJson),
    ],
  );
  ...
  factory Player.start(
    ...
    usePartnerB: false,
    states: [
      PlayerState.start(life: startingLife, others: others, counters: counters),
    ],
  );
  ...
  void applyAction(PlayerAction action) {
    checkFirstStateTime();
    states.add(states.last.applyAction(action));
  }
  ...
  void insertAction(
    ...
    List<PlayerAction> actions = [
      for (int i = states.length - 1; i >= stateIndex; --i) back(counterMap),
    ];
