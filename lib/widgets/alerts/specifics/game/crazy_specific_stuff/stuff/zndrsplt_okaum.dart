...
class _ZndrspltOkaumState extends State<_ZndrspltOkaum> {
  ...
  void solveFlip(bool choice) {
    ...
    if (choice) {
      ...
      /// next commander's trigger if any
    }
  ...
  void autoSolveTriggers() {
    ...
    while (steps < 100 && currentFlip != null) {
      ...
      /// security check, don't want to enter an infinite loop
                ...
                if (currentFlip != null)
                  Expanded(
                    ...
                    child: SubSection(<Widget>[
                      SectionTitle(
                          "Current flip ${currentFlip!.flips.length > 1 ? '(${currentFlip!.flips.length} thumb-coins)' : ""}"),
                      if (currentFlip!.flips.length > 1)
                        ButtonTilesRow(
                          children: <Widget>[
                            ButtonTile.transparent(
                              icon: null,
                        ...
                        ButtonTilesRow(
                          children: <Widget>[
                            ButtonTile(
                              icon: currentFlip!.containsWin
                                  ? Icons.check
                                  : Icons.close,
                              onTap: null,
                              text: currentFlip!.containsWin
                                  ? "Heads\n(win)"
                                  : "Tails\n(loss)",
                              twoLines: true,
                            ),
          ...
          SubSection(
            [
              ListTile(
                ...
                title: AnimatedText((currentFlip == null
                        ? "Begin Combat"
                        : "Can't begin combat") +
                    (alwaysTryToWin ? " (auto)" : " (manual)")),
                subtitle: currentFlip == null
                    ? null
                    : const Text("Solve triggers first"),
...
class _ThumbFlip {
  ...
  _ThumbFlip(int howManyThumbs, Random? rng)
      : flips = [
