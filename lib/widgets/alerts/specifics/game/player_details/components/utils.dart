...
class DetailsUtils {
  static void insertLife(StageData stage, String name, CSBloc bloc,
          PlayerState playerState, List<String> names) =>
      stage.showAlert(
          InsertAlert(
  ...
  static void insertCounter(Counter counter, StageData stage, String name,
          CSBloc bloc, PlayerState playerState, List<String> names) =>
      stage.showAlert(
          InsertAlert(
              inputType: TextInputType.number,
              ...
              onConfirm: (string) {
                ...
                if (val != null) {
                  bloc.game.gameState.applyAction(GACast(
                    ...
                    usingPartnerB: {
                      for (final n in names)
                        if (n == name) n: partnerB else n: false,
                    },
                    maxVal: bloc.settings.gameSettings.maxValue.value,
                  ));
                }
  ...
  static void insertDamage(bool havePartner, bool partnerB, StageData stage,
          String attacker, String defender, CSBloc bloc, GameState? gameState,
          {bool replace = false}) =>
      stage.showAlert(
          InsertAlert(
              inputType: TextInputType.number,
              ...
              onConfirm: (string) {
                ...
                if (val != null) {
                  bloc.game.gameState.applyAction(GADamage(
                    val -
                        gameState!
                            .players[defender]!.states.last.damages[attacker]!
                            .fromPartner(!partnerB),
                    defender: defender,
                    attacker: attacker,
                    ...
                    settings: gameState.players[attacker]!
                        .commanderSettings(!partnerB),
                  ));
                }
              }),
          size: InsertAlert.twoLinesHeight,
          replace: replace);

  static void partnerDamage(StageData stage, String attacker, String defender,
          CSBloc bloc, GameState? gameState) =>
      stage.showAlert(
        AlternativesAlert(
          ...
          alternatives: <Alternative>[
            Alternative(
                ...
                action: () {
                  insertDamage(
                      true, false, stage, attacker, defender, bloc, gameState,
                      replace: true);
                }),
            Alternative(
                ...
                action: () {
                  insertDamage(
                      true, true, stage, attacker, defender, bloc, gameState,
                      replace: true);
                }),
          ],
        ),
  ...
  static void renamePlayer(
          StageData stage, String name, CSBloc bloc, List<String> names) =>
      stage.showAlert(
          InsertAlert(
...
class PlayerBuilder extends StatelessWidget {
  final int index;
  final Widget Function(GameState, List<String>, String, PlayerState, Player)
      builder;
  const PlayerBuilder(this.index, this.builder, {super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return groupBloc.orderedNames.build(
      (_, names) {
        ...
        return stateBloc.gameState.build(
          (_, state) {
