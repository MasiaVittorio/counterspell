...
class CommanderStats {
  ...
  factory CommanderStats.fromPastGames(
      MtgCard card, Iterable<PastGame?> pastGames) {
    int present = 0;
    int winner = 0;
    int totalDamage = 0;
    int totalCasts = 0;
      ...
      if (game != null) {
        if (game.winner != null && game.commanderPlayed(card)) {
          ...
          for (final pilot in pilots) {
            ...
            final bool partnerA =
                game.commandersA[pilot]?.oracleId == card.oracleId;
            averageDamageThisGame +=
                game.state.totalDamageDealtFrom(pilot, partnerA: partnerA);
          }
          ...
          for (final pilot in pilots) {
            ...
            final bool partnerA =
                game.commandersA[pilot]?.oracleId == card.oracleId;
            averageCastsThisGame += game.state.players[pilot]!.states.last.cast
