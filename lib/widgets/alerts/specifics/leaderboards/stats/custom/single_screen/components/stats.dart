...
class Stats extends StatelessWidget {
  ...
  const Stats(this.stat, {super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final plFreq = stat.playersApplicable.isNotEmpty
        ? stat.playersApplicable.entries.reduce(
            (a, b) => (a.value / players[a.key]!.games) >
                    (b.value / players[b.key]!.games)
                ? a
                : b,
          )
        : null;
    final int? plFreqGames = plFreq != null ? players[plFreq.key]!.games : null;
    final double? plFreqValue =
        plFreq != null ? plFreq.value / plFreqGames! : null;
    ...
    final cmdrTot = stat.commandersApplicable.isNotEmpty
        ...
        : null;
    final cmdrTotCard = cmdrTot != null ? cmdrs[cmdrTot.key!]?.card : null;

    final cmdrFreq = stat.commandersApplicable.isNotEmpty
        ? stat.commandersApplicable.entries.reduce(
            (a, b) => (a.value / cmdrs[a.key!]!.games) >
                    (b.value / cmdrs[b.key!]!.games)
                ? a
                : b,
          )
        : null;
    final cmdrFreqCard = cmdrFreq != null ? cmdrs[cmdrFreq.key!]!.card : null;
    final int? cmdrFreqGames =
        cmdrFreq != null ? cmdrs[cmdrFreq.key!]!.games : null;
    final double? cmdrFreqValue =
        cmdrFreq != null ? cmdrFreq.value / cmdrFreqGames! : null;

    final double rate =
        stat.appearances == 0 ? 0 : 100 * stat.wins / stat.appearances;
            ...
            [
              ...
              if (plFreq != null)
                ListTile(
                  ...
                  subtitle: Text(
                      "Most frequent (${plFreq.value} over $plFreqGames games)"),
                  trailing:
                      Text("${InfoDisplayer.getString(plFreqValue! * 100)}%"),
                ),
              if (cmdrTot != null)
                ListTile(
              ...
              if (cmdrFreq != null)
                ListTile(
                  ...
                  subtitle: Text(
                      "Most frequent (${cmdrFreq.value} over $cmdrFreqGames games)"),
                  trailing:
                      Text("${InfoDisplayer.getString(cmdrFreqValue! * 100)}%"),
                ),
            ],
            margin: margin,
          ),
