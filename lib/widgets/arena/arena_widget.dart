...
import 'package:counter_spell/widgets/arena/components/all.dart';
                      ...
                      groupLogic.arenaNameOrder.build((context, positions) =>
                          groupLogic.orderedNames.build((context, names) =>
                              arenaBloc.flipped.build((context, flipped) =>
                                  StageBuild.offMainPage<CSPage>(
                                    (_, page) => scrollerBloc.isScrolling.build(
                                      (context, scrolling) => ArenaLayout(
    ...
    final Widget button = ArenaMenuButton(
      ...
      reorderPlayers: () => setState(() {
        ...
        groupLogic.arenaNameOrder.set(<int, String?>{
          for (final key in groupLogic.arenaNameOrder.value.keys) key: null,
        });
      }),
      screenConstraints: screenConstraints,
    );

    final Widget delayer = logic.settings.scrollSettings.confirmDelay.build(
      (context, delay) => ArenaDelayer(
