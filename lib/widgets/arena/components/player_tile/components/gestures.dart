...
class AptGestures extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: StageBuild.offMainEnabledPages(
        (_, enabled) => bloc.settings.arenaSettings.scrollOverTap.build(
          (context, scrollOverTap) {
            if (scrollOverTap) {
              ...
              final Widget arrows = settings.verticalScroll.build(
                (context, verticalScroll) {
                  ...
                  return Flex(
                    ...
                    children: <Widget>[
                      if (verticalScroll) ...[children[true]!, children[false]!]

                      /// top + bottom -
                      else ...[children[false]!, children[true]!],
            ...
            final Widget buttons = settings.verticalTap.build(
              (context, verticalTap) {
                final Map<bool, Widget> children = <bool, Widget>{
                  for (bool plus in const <bool>[true, false])
                    plus: Expanded(
                      child: ContinuousPressInkResponse(
                        containedInkWell: true,
                        onTap: () {},

                        /// not null because the inkwell must be seen as enabled
                        onTapDown: (_) => tapOnly(plus, stage, 1),
                        whileLongPress: (ticks) => tapOnly(
                          plus,
                          stage,
                          switch (ticks) {
                            >= 0 && <= 4 => 1,
                ...
                return Flex(
                  ...
                  children: <Widget>[
                    if (verticalTap) ...[children[true]!, children[false]!]
  ...
  void tapOnly(bool topHalf, StageData? stage, int increment) {
...
class AptCmdrDmg extends StatelessWidget {
  ...
  static const double _size = 56.0 + 2 * AptGestures.aptCmdrDamageExtraPadding;
