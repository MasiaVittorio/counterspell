...
class CSBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (_, constraints) => ConstrainedBox(
      ...
      child: SingleChildScrollView(child: BlocVar.build5
      <Map<CSPage,Color>?,bool,Color,bool,List<String>>(
        stageTheme.derived.mainPageToPrimaryColor, 
        themer.flatDesign, 
        themer.defenceColor, 
        bloc.tutorial.showingTutorial, 
        group.orderedNames,
        builder: (_, pageColors, flat, defenceColor, showingTutorial, ordered){

          final Set<String> current = {...ordered};
          final updated = bloc.game.gameState.gameState.value.names;
          late final List<String> names;
          if(!updated.containsAll(current) || !current.containsAll(updated)){
            names = [...updated];
          } else {
            names = ordered;
          }
                    ...
                    AnimatedPositioned(
                      ...
                      child: Material(
                        ...
                        child: BodyGroup(
                          ...
                          secondHighlightableIndex: secondHighlightableIndex,
                          flat: flat,
                        ),
                      ),
                    ),
