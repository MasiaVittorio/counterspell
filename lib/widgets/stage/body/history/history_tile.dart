...
import 'package:counter_spell/widgets/stage/body/history/current_state_tile.dart';
import 'package:counter_spell/widgets/stage/body/history/history_player_tile.dart';
...
class HistoryTile extends StatelessWidget {
  ...
  Widget buildKnowingSize(
      double? knownTileSize, StageData stage, CSBloc logic) {
    ...
    return logic.themer.flatDesign.build(
                    ...
                    stage.showAlert(
                      AlternativesAlert(
                        alternatives: [
                          ...
                          Alternative(
                            title:
                                "Merge with previous action ($index - ${index + 1})",
                            icon: Icons.merge,
                            action: () {
              ...
              children: CSSizes.separateColumn(
                ...
                <Widget>[
                  for (final name in names)
                    if (data.changes!.containsKey(name))
                      HistoryPlayerTile(
                        data.changes![name]!,
                        time: data.time,
                        firstTime: firstTime,
