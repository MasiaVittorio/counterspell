...
import 'package:counter_spell/widgets/stage/panel/extended_components/all.dart';

class PagePie extends StatelessWidget {
  const PagePie({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: StageBuild.offMainEnabledPages(
        (_, enabled) =>
            stage!.themeController.derived.mainPageToPrimaryColor.build(
          (_, colors) => Column(
            children: <Widget>[
              Padding(
                padding: const EdgeInsets.only(bottom: 2.0),
                child: pageLabel(CSPage.values.first, theme, stage,
                    enabled as Map<CSPage, bool>),
              ),
              Expanded(
                child: CircularLayout(
                  ...
                  labels: <Widget>[
                    for (final page in CSPage.values)
                      page == CSPage.values.first
                          ? Container()
                          : pageLabel(page, theme, stage, enabled),
                  ],
                  clockWise: false,
                  labelPadding: 12.0,
                ),
  ...
  Widget pageButton(
    ...
    Color unselected,
      ...
      Material(
        ...
        elevation:
            enabled[page]! && PanelSettings.disablablePages.contains(page)
                ? 4
                : 0,
