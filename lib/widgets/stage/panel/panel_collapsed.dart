...
class CSPanelCollapsed extends StatelessWidget {
  const CSPanelCollapsed({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return ArenaTransformer(
      ...
      builder: (_, open) => StageBuild.offMainPage<CSPage>(
        (_, currentPage) => StageBuild.offMainColors((_, _, colors) {
          ...
          final rightButton = bloc.scroller.isScrolling.build(
            (context, scrolling) => CircleButton(
              ...
              color: colors![currentPage]!.withValues(alpha: 0.07),
              ...
              child:
                  ...
                  const SizedBox(width: CSSizes.barSize),
              onTap: () {
                if (currentPage == CSPage.history) {
                  stage.showSnackBar(const SnackRestart(), rightAligned: true);
          ...
          final backAndForward = Row(
            ...
            children: [backButton, forwardButton],
          );
      ...
      builder:
          (
            ...
            final accentColor = Color.alphaBlend(
              currentPrimaryColor.withValues(alpha: 0.85),
              canvas,
            );
